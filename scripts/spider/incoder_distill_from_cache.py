...
if __name__ == "__main__":
    ...
    model_out_path = None
    data_path = "../../outputs/incoder_cache_distill_test1/"
    ...
    for db in dbs:
        ...
        with open(os.path.join(data_path, 'distill_data_%s_0.pkl' % (db)), 'rb') as f:
            distill_items.append(pkl.load(f))
        
        with open(os.path.join(data_path, 'distill_data_%s_1.pkl' % (db)), 'rb') as f:
            distill_items.append(pkl.load(f))
        
        # combined_distill_data = sum(distill_items)
        # injection_data = injection_data_set[0]
        for curr_injection_idx, (distill_data, injection_data) in list(enumerate(zip(distill_items, injection_data_set))):
            if curr_injection_idx == 0:
                continue
