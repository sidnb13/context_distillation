...
from incoder_spider_data import create_spider_injection_data, create_spider_injection_data2, load_spider
...
if __name__ == "__main__":
    ...
    n_per_prompt = 4
    n_prompts = 2
    n_questions_per_prompt = 8

    injection_datas = create_spider_injection_data2('../../data/spider/dev.json', '../../data/spider/db_id2schema_2.pkl', n_per_prompt, n_prompts, n_questions_per_prompt, 0, add_description, grad_descent_eval_mode=False)
    ...
    for db, injection_data_set in injection_datas.items():
        ...
        for curr_injection_idx, injection_data in enumerate(injection_data_set):
            ...
            data_out_path = "../../outputs/incoder_injectionpromptvgradientdescent_spider_test7/tk_injection_add_description_%s_%d_datapoints_%d_per_prompt____%s/" % (str(add_description), n_per_prompt*(curr_injection_idx+1), n_per_prompt, db)
            ...
            with open(os.path.join(data_out_path, 'questions.pkl'), 'wb') as f:
                pkl.dump(questions[(curr_injection_idx)*(len(questions) // len(injection_data_set)):(curr_injection_idx+1)*(len(questions) // len(injection_data_set))], f)
