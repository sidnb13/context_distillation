...
if __name__ == "__main__":
    ...
    injection_datas = create_spider_injection_data2('../../data/spider/dev.json', '../../data/spider/db_id2schema.pkl', n_per_prompt, n_prompts, n_questions_per_prompt, 0, add_description, grad_descent_eval_mode=False)
    ...
    for db, injection_data_set in injection_datas.items():
        for curr_injection_idx, injection_data in enumerate(injection_data_set):
            ...
            data_out_path = "../../outputs/incoder_injectionpromptvgradientdescent_spider_test4/tk_injection_add_description_%s_%d_datapoints_%d_per_prompt____%s/" % (str(add_description), n_per_prompt*(curr_injection_idx+1), n_per_prompt, db)
