...
if __name__ == "__main__":
    ...
    data_out_path = "../../outputs/fact_edit_from_trained_teacher_from_input_synth_test2/"
    ...
    teacher_checkpoint = 'outputs/trained_counterfact_teacher/model/'
    student_checkpoint = 'outputs/trained_counterfact_teacher/model/'
    input_synth_path = 'outputs/T5_11B_fact_edit_input_generator2/model/'
    ...
    with seed_context(random_state):
        ...
        # injection_data = injection_data[:4]
    ...
    for item in injection_data:
        ...
        main_fact_prompt = item.meta['requested_rewrite']['prompt'].replace('{}', item.meta['requested_rewrite']['subject'])
        new_fact = f"{main_fact_prompt} {item.meta['requested_rewrite']['target_new']['str']}"
        old_fact = f"{main_fact_prompt} {item.meta['requested_rewrite']['target_true']['str']}"
        
        question_prompt_generation = f"Definition: In this task, you will generate diverse and high quality input queries for training a language model to edit the fact \"{old_fact}\" to \"{new_fact}\". Now complete the following example - Input: generation Output:"
        question_prompt_neighborhood = f"Definition: In this task, you will generate diverse and high quality input queries for training a language model to edit the fact \"{old_fact}\" to \"{new_fact}\". Now complete the following example - Input: neighborhood Output:"
        question_prompt_attribute = f"Definition: In this task, you will generate diverse and high quality input queries for training a language model to edit the fact \"{old_fact}\" to \"{new_fact}\". Now complete the following example - Input: attribute Output:"
        question_prompt_paraphrase = f"Definition: In this task, you will generate diverse and high quality input queries for training a language model to edit the fact \"{old_fact}\" to \"{new_fact}\". Now complete the following example - Input: paraphrase Output:"
        
        # item = replace(item, question_generation_prompts=[item.question_generation_prompts[0][:item.question_generation_prompts[0].find(" Positive Example 1 - ")]+" Now complete the following example - Input: . Output:"])
        item = replace(item, question_generation_prompts=[question_prompt_generation, question_prompt_neighborhood, question_prompt_attribute, question_prompt_paraphrase])
    ...
    for item, questions in zip(injection_data, all_questions):
        ...
        if jax.process_index() == 0:
            wandb_state = wandb.init(project="distill_facts", reinit=True)
        ...
        for _ in range(2):
            ...
            rng_key, new_rng = jax.random.split(rng_key)
            trainer = distill(distill_data, trainer, mesh, bsize=8, epochs=1, max_input_length=256, 
                              rng_key=new_rng, decompress_smoothing_epsilon=1e-7)
