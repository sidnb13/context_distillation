...
class FlaxXGLMDecoderLayerCollection(nn.Module):
    ...
    def setup(self):
        block = FlaxXGLMDecoderLayer
        if self.config.gradient_checkpointing:
            FlaxXGLMCheckpointDecoderLayer = remat(block, static_argnums=(4, 5, 6))
            block = FlaxXGLMCheckpointDecoderLayer
        self.layers = [
            block(self.config, name=str(i), dtype=self.dtype) for i in range(self.config.num_layers)
        ]
